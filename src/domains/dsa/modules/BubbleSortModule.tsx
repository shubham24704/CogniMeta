...
export function BubbleSortModule() {
  ...
  const steps = useMemo(() => bubbleSortSteps(input), [input]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <h1 className="text-4xl font-semibold text-[var(--cm-ink)] mb-2">{bubbleSortContent.title}</h1>
        <p className="text-[var(--cm-ink-soft)] max-w-2xl">{bubbleSortContent.tagline}</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          <ComplexityPanel
            best={bubbleSortContent.complexity.best}
            average={bubbleSortContent.complexity.average}
            worst={bubbleSortContent.complexity.worst}
            space={bubbleSortContent.complexity.space}
            intuition={bubbleSortContent.intuition}
            comparisons={current.state.stats.comparisons}
            swaps={current.state.stats.swaps}
          />
          <CodeViewer code={bubbleSortContent.code} />
        </div>
      </div>
    </div>
  );
